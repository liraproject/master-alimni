<div x-show="showDrawer" x-data="{ showDrawer: false }"
    x-on:show-profile-drawer.window="showDrawer = true"
    ...
    <div class="fixed right-0 top-0 z-[101] h-full w-72">
        <div class="flex flex-col w-full h-full transition-transform duration-200 bg-white transform-gpu dark:bg-navy-700"
            ...
            <div class="h-24">
                <img class="object-cover object-center w-full h-full" src="{{ asset('images/800x600.png') }}"
                    alt="image" />
            </div>
            <div class="flex px-5 space-x-5">
                <x-web.elements.avatar
                    initial="{{ substr($user->fullname, 0, 2) }}" rounded="full"
                    variant="circle" color="primary" size="xl" class="-mt-5"
                />
                <div class="w-full mt-2">
                    ...
                    <a href="#"
                        class="cursor-pointer text-xs+ text-primary hover:text-primary-focus dark:text-accent-light dark:hover:text-accent">{{ $user->email }}</a>
                </div>
            </div>
            ...
            <div class="p-5">
                <button @click="showDrawer = false"
                    class="w-full space-x-2 font-medium btn bg-warning/10 text-warning hover:bg-warning/20 focus:bg-warning/20 active:bg-warning/25">
                    <svg xmlns="http://www.w3.org/2000/svg" class="size-5" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
